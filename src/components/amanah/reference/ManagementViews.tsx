...
function DecisionSignal({ title, value, note, action, tone = "gold" }: { title: string; value: string; note: string; action: string; tone?: Tone }) {
  ...
  return <article className={`rounded-[1rem] border ${stripe} bg-white/[0.025] p-4 text-right`}><p className="text-[11px] text-white/46">{title}</p><div className="mt-2 flex items-end justify-between gap-3"><span className="text-[11px] font-bold text-white/48">{action}</span><p className={`font-['Tajawal'] text-[1.65rem] font-black leading-none ${valueTone}`}>{value}</p></div><p className="mt-3 text-[11px] leading-6 text-white/62">{note}</p></article>;
}
...
export function ReportsReference() {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 2xl:grid-cols-[1.18fr_0.82fr]">
      <Surface title="القراءة التنفيذية لهذا الأسبوع" subtitle="ما الذي تغيّر ويستحق العرض"><div className="grid gap-3 sm:grid-cols-3"><DecisionSignal title="قيمة الفرص الجاهزة" value="67.9 م" note="6 فرص تستوفي حد الجاهزية للعرض التنفيذي" action="ارتفاع 12%" tone="green" /><DecisionSignal title="الجاهزية التنظيمية" value="83%" note="تجاوز المستهدف المحدد بـ 6 نقاط" action="استمر بالرفع" tone="blue" /><DecisionSignal title="قرار مطلوب" value="2" note="ملفان متأخران قد يؤثران على دورة الطرح" action="عرض اللجنة" tone="red" /></div></Surface>
